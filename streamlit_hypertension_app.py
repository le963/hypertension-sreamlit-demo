...
import shap, joblib
plt.ioff()  # 关闭matplotlib交互模式，加快渲染
...
def risk_level(p):
    return "低风险" if p < 0.3 else ("中风险" if p < 0.7 else "高风险")
...
if menu == "单人风险预测":
    ...
    if st.button("🚀 开始风险评估"):
        with st.spinner("正在计算..."):
            ...
            person_data = {"Age": age,"Salt_Intake": salt,"Stress_Score": stress,"Sleep_Duration": sleep,
                           "BMI": bmi,"Exercise_Level": exercise,"Family_History": family_history,
                           "BP_History": bp_history_raw,"Smoking_Status": smoke_status_raw}
            ...
            name_mapping = {"Age":"年龄","Salt_Intake":"盐摄入量","Stress_Score":"压力评分","Sleep_Duration":"睡眠时长","BMI":"BMI",
            ...
            for feat_name, shap_val in shap_dict.items():
                ...
                if feat_name in ["Age","Salt_Intake","Stress_Score","Sleep_Duration","BMI","Exercise_Level"]:
                    ...
                    if abs(shap_val) > 0.01:
                        if desc in bad_desc_set: risk_desc_list.append(desc)
                        elif desc in good_desc_set: protect_desc_list.append(desc)
                        elif risk_by_shap: risk_desc_list.append(desc)
                        else: protect_desc_list.append(desc)
                elif feat_name == "Family_History":
                    val = raw_data[feat_name]
                    desc = "存在高血压家族病史" if val == 1 else "无高血压家族病史"
                    if abs(shap_val) > 0.01:
                        risk_desc_list.append(desc) if val == 1 else protect_desc_list.append(desc)
            if bp_history == "高血压": risk_desc_list.append("既往存在高血压病史")
            elif bp_history == "高血压前期": risk_desc_list.append("既往处于高血压前期")
            else: protect_desc_list.append("既往自述血压正常")
            if smoke_status == "有吸烟习惯": risk_desc_list.append("有吸烟习惯")
            else: protect_desc_list.append("无吸烟习惯")
...
# ==================== 多人在线录入批量预测 ====================
    ...
    if "multi_person_df" not in st.session_state:
        st.session_state.multi_person_df = pd.DataFrame([
            {"姓名":"人员A","Age":42.0,"Salt_Intake":4.5,"Stress_Score":4.0,"Sleep_Duration":7.0,"BMI":22.5,
             "Exercise_Level":"中","Family_History":"无高血压家族史","BP_History":"正常","Smoking_Status":"没有吸烟习惯"},
            {"姓名":"人员B","Age":55.0,"Salt_Intake":7.2,"Stress_Score":7.0,"Sleep_Duration":6.0,"BMI":26.1,
             "Exercise_Level":"低","Family_History":"有高血压家族史","BP_History":"高血压前期","Smoking_Status":"有吸烟习惯"},
        ])
    edited_df = st.data_editor(
        st.session_state.multi_person_df,
        num_rows="dynamic", use_container_width=True, key="multi_editor",
        column_config={
    ...
    with col_btn2:
        st.download_button(label="📥下载模板CSV", data=edited_df.to_csv(index=False).encode("utf‑8‑sig"),
                           file_name="多人录入模板.csv", mime="text/csv")
    if run_batch:
        ...
        st.download_button(label="📥下载预测结果CSV",
                           data=output_df.to_csv(index=False).encode("utf‑8‑sig"),
                           file_name="多人批量预测结果.csv", mime="text/csv")
        ...
        if st.button("批量运行模型预测"):
            with st.spinner("正在批量计算..."):
                ...
                if df_valid.shape[0]==0:
                    ...
                    batch_prob = model.predict_proba(batch_scaled)[:,1]
                    result_df = pd.DataFrame({"患病预测概率(%)": np.round(batch_prob*100,2),
                                              "风险等级": [risk_level(p) for p in batch_prob]})
            st.subheader("批量预测结果")
            st.dataframe(result_df, use_container_width=True)
            st.download_button(label="📥下载批量预测结果",
                               data=result_df.to_csv(index=False).encode("utf‑8‑sig"),
                               file_name="csv上传批量预测结果.csv", mime="text/csv")
            st.subheader("🔍全局SHAP特征贡献图")
